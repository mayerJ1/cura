...
#include "MATHH.h" // for M_PI. Use relative path to avoid pulling <math.h>
...
INLINE double vSizeMM(const Point& p0)
{
    ...
    return sqrt(static_cast<float>(fx*fx+fy*fy));
}
...
INLINE Point rotate(const Point& p0, float angle)
{
...
public:
    ...
    PointMatrix(const Point p)
    {
        matrix[0] = p.X;
        matrix[1] = p.Y;
        double f = sqrt(static_cast<float>((matrix[0] * matrix[0]) + (matrix[1] * matrix[1])));
        matrix[0] /= f;
        matrix[1] /= f;
        matrix[2] = -matrix[1];
        matrix[3] = matrix[0];
